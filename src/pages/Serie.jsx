  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: of }, { data: pr }] = await Promise.all([
        supabase.from('oficial').select('*').eq('serie', 'A').eq('rodada', rodadaSel).order('pos'),
        supabase.from('projecao').select('*').eq('serie', 'A').eq('rodada', rodadaSel).order('pos'),
      ])
      setOficial(of || [])
      setProjecao(pr || [])
      setLoading(false)
            ...
            <>
                ...
                <div className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="font-semibold text-white">
                      Classificação Oficial · Rodada {rodadaSel}
                    </h2>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {oficial.map(row => {
                          ...
                          const isLib    = row.pos >= 2 && row.pos <= 4
                          const isPreLib = row.pos === 5
                          const isSul    = row.pos >= 6 && row.pos <= 12
                          const isRel    = row.pos >= 17

                          const rowBg = isChamp ? 'bg-yellow-400/5'
                            : isLib    ? 'bg-green-400/5'
                            : isPreLib ? 'bg-yellow-400/5'
                            : isSul    ? 'bg-blue-400/5'
                            : isRel    ? 'bg-red-400/5' : ''

                          const leftBorder = isChamp  ? 'border-l-4 border-yellow-400'
                            : isLib    ? 'border-l-4 border-green-500'
                            : isPreLib ? 'border-l-4 border-yellow-500'
                            : isSul    ? 'border-l-4 border-blue-500'
                            : isRel    ? 'border-l-4 border-red-500'
                            : 'border-l-4 border-transparent'

                          return (
                            <tr key={row.time}
                                className={`border-t border-gray-800 ${rowBg} ${leftBorder}`}>
                              ...
                              <td className={`px-4 py-3 text-center font-medium ${
                                row.sg > 0 ? 'text-green-400' : row.sg < 0 ? 'text-red-400' : 'text-gray-400'
                              }`}>
                                {row.sg > 0 ? '+' : ''}{row.sg}
                              </td>
                            </tr>
              ...
              {tab === 'projecao' && (
                <div className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800">
                  <div className="px-6 py-4 border-b border-gray-800">
                    <h2 className="font-semibold text-white">
                      Projeção Monte Carlo · Rodada {rodadaSel}
                    </h2>
                    <p className="text-gray-400 text-xs mt-1">
                      10.000 simulações · Poisson + Gaussian Phase Factor
                    </p>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-gray-400 text-xs uppercase tracking-wider">
                          ...
                          <th className="px-4 py-3 text-center">% Libert.</th>
                      ...
                      <tbody>
                        {projecao.map(row => {
                          ...
                          return (
                            <tr key={row.time}
                                className={`border-t border-gray-800 ${style.bg} ${style.border}`}>
                              <td className="px-4 py-3 text-gray-400 font-mono">{row.pos}</td>
